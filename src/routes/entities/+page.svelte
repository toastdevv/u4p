...
<header class="semi-screen flex w-full gap-8 bg-primary-900 p-16 drop-shadow-lg">
	<div class="flex h-full w-full flex-shrink flex-col drop-shadow-lg">
		...
		<div class="flex h-full w-full flex-row gap-8">
			<a
				href="/"
				class="aspect-video h-full w-full bg-slate-300 transition-all hover:rounded-md hover:border-2 hover:border-red-700"
			>
			</a>
			<div>
...
<main class="flex flex-col justify-evenly p-24">
	<Tabs
		style="full"
	...
	>
		{#each ['Individuals', 'Professionals', 'Organizations'] as entity, i (i)}
			<TabItem open={i == 0} class="w-full">
				<span slot="title" class="text-lg font-semibold">{entity}</span>
